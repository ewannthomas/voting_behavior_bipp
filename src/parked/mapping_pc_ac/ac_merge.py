...
ac_shp["st_name"] = np.where(
    ac_shp["pc_name"].isin(telangana_pcs), "telangana", ac_shp["st_name"]
)
...
# cleaning ancilliary columns of df

df["election"] = np.where(
    df["election"] == "vidhan sabha", "Vidhan Sabha", df["election"]
)


conds = [
    (df["internal_ac_pc"].isin(["Yes", "yes"])),
    (df["internal_ac_pc"].isin(["No", "no"])),
]
opts = [1, 0]
df["internal_ac_pc"] = np.select(conds, opts, default=df["internal_ac_pc"])


conds = [
    (df["interim"].isin(["Yes", "yes"])),
    (df["interim"].isin(["No", "no"])),
]
opts = [1, 0]
df["interim"] = np.select(conds, opts, default=df["interim"])

for col in ["election", "internal_ac_pc", "interim"]:
    print(df[col].unique())
